...
@cache_cli.command()
@click.option("--cache-type", "-t", help="Type of cache to clear (repos, refs, metadata, scripts)")
...
@cache_cli.command()
@click.option("--max-age-days", "-a", default=30.0, help="Maximum age for cache entries in days")
def cleanup(max_age_days: float) -> None:
    """Clean up expired cache entries."""
    cache = get_cache()
    cache.cleanup_expired(max_age_days)
    console.print(f"[green]✅ Cleaned up cache entries older than {max_age_days} days[/green]")
