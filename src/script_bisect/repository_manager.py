...
class RepositoryManager:
    ...
    def setup_repository(self, good_ref: str, bad_ref: str) -> Path:
        ...
        with Progress(
            ...
            try:
                ...
                progress.update(task, description="Configuring for optimal performance...")
                
                # Enable sparse-checkout for minimal working directory
                self.repo.git.config("core.sparseCheckout", "true")
                sparse_checkout_path = self.clone_dir / ".git" / "info" / "sparse-checkout"
                ...
                try:
                    ...
                    logger.debug("Range fetch failed, trying to fetch all refs with blob filtering")
                ...
                raise git.GitCommandError(
                    f"Failed to set up repository: {e}", 
                    status=1
                ) from e
    ...
    def get_commit_range(self, good_ref: str, bad_ref: str) -> list[str]:
        ...
        try:
            ...
            raise ValueError(f"Cannot get commit range {good_ref}..{bad_ref}: {e}") from e
